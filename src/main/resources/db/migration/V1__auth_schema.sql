...
CREATE UNIQUE INDEX ux_auth_users_email_lower ON auth_users (LOWER(email));
CREATE INDEX idx_auth_users_telefono ON auth_users (telefono);
...
-- Datos semilla: admin@redpatitas.com / password (BCrypt, coste 10 - compatible con verificación BCrypt)
...
INSERT INTO auth_users (id, nombre, apellido, telefono, email, password_hash, enabled, failed_login_attempts, created_at, updated_at)
VALUES (
